...
load(here::here("wtp", "wtp_ci_mxl.RData"))
...
# MNL
taxcred_taxfile_value <- filter(wtp_ci, par == "type_taxCredit_taxFiling")$mean
...
# MNL
rebate6wks_value <- filter(wtp_ci, par == "type_rebate_6wks_govt")$mean
...
# MNL
rebate2wks_value <- filter(wtp_ci, par == "type_rebate_2wks_govt")$mean
...
evsales_taxcred <- evsales_taxcred %>%
  mutate(
    taxcred_adj = taxcredit + (wtp_ci$mean),
    taxcred_spend = sales * taxcredit,
    taxcred_spend_adj = sales * (taxcredit + (wtp_ci$mean)),
...
col_value <- "grey42"
col_loss <- "#FF3B3F"
evsales_taxcred_plot <- evsales_taxcred %>% 
  filter(year != '2010') %>% 
  ggplot() +
