...
raw_data_start <- read_csv(here("data", "Vehicle_Incentive_Study_Dynata_start_50k.csv"))
raw_data_prac <- read_csv(here("data", "Vehicle_Incentive_Study_Dynata_prac_50k.csv"))
raw_data_main <- read_csv(here("data", "Vehicle_Incentive_Study_Dynata_50k.csv"))
raw_data_demos <- read_csv(here("data", "Vehicle_Incentive_Study_Dynata_Demos_50k.csv"))
...
raw_data_start <- raw_data_start %>%
  ...
  select(session, psid, respondentID, inMarket, income, inMarketscreenout, incomescreenout, sec_elapsed_start)
...
data <- raw_data %T>%
  ...
  # 4- Remove income screenouts
  filter(incomescreenout == 0) %T>%
  {print(paste("income screenout",nrow(.)))} %>%
  # 5 - practice
  filter(practice == 4 ) %T>%
  {print(paste("practice screenout",nrow(.)))} %>%
  # filter out bad responses
  # 6 - Remove cases that did not complete the CBC questions
  filter(!is.na(cbc1)) %>% 
  filter(!is.na(cbc2)) %>% 
  filter(!is.na(cbc3)) %>% 
  filter(!is.na(cbc4)) %>% 
  ...
  # 7- Remove if all conjoint responses were the same
  filter(cbcAllSame == 0 ) %T>%
  {print(paste("cbc screenout",nrow(.)))}
  # 8- Didnt complete full survey
...
print(paste(length(unique(data$respondentID)), "vs.", nrow(data)))
...
tabyl(data$carBudget)

tabyl(data$test_tax_credit)

tabyl(data$test_tax_deduct)
...
tabyl(data$considerBEV)

tabyl(data$householdVehicles)

tabyl(data$parkingHousehold)
...
tabyl(data$education)

tabyl(data$new_or_used)
...
zips_comb_geo <- geocode_zip(zips_comb$zipcode) 
